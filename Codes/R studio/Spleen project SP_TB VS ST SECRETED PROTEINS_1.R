...
data.pr.all = read.csv("F:/Proteomics/Data/Spleen project_SP/Excel sheets/7. Spleen project _SP_Secreted proteins.csv")
annotation.all = data.pr.all[,4:5]
data.pr = data.pr.all[,6:17]
data.pr.log2 = log2(data.pr)
rownames(data.pr.log2) = annotation.all$Accession
conditions = factor(c("ST","ST","ST","ST","ST","ST","TB","TB","TB","TB","TB","TB"))
...
pheatmap(data.pr.log2.2, cluster_rows=F, show_rownames=T,scale='row',
         cluster_cols=T,main="all protein heatmap",annotation_col = df,color=colorRampPalette(c("#D91A1A","#B41C1C","#8F1D1E","#6B1F20","#010203","#014122","#018141","#00C060","#00FF7F"))(100)) 
dev.off()  
...
thr.pv=0.05
diffexp.pr.all = topTable(eb, adjust="BH",coef = 2,number = dim(data.pr.log2.2)[1])
...
write.table(diffexp.pr.all,file="TB vs ST ALL PROTEINS.csv",sep=",")
